...
count.publish(10) // here your `alert()` would be called
```
...
These events will be executed within the scope of the trigger, except it occurs before or after the trigger has achieved
its task.
...
the publish method has published your changes.

The above is true for the other events as well, such as `set` and `broadcast`.
...
a `superstate` has, such as `publish`, `set`, `discard`, etc [(Learn more)](/api-reference/@superstate/core/superstate). To illustrate you an example:
...
The Middleware above will assign to your draft the value of `now()`, which is `5`, every time you `.publish()` something.
